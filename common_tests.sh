...
function assert_RPM_REPO_BASE_URL {
  ...
  local msg="RPM_REPO_BASE_URL for (USE_TEST_REPO=$USE_TEST_REPO, ENVIRONMENT=$ENVIRONMENT) should be $expected"
  assert_eq "$expected" "$RPM_REPO_BASE_URL" "$msg" && log_success "$msg" || TESTS_RESULT=$?
}

log_header "Tests for RPM_REPO_BASE_URL"
assert_RPM_REPO_BASE_URL "true" "live" "https://repository.hazelcast.com/rpm-test-local"
assert_RPM_REPO_BASE_URL "true" "sandbox" "https://repository.hazelcast.com/rpm-test-local"
assert_RPM_REPO_BASE_URL "false" "live" "https://repository.hazelcast.com/rpm-local"
assert_RPM_REPO_BASE_URL "false" "sandbox" "https://repository.hazelcast.com/sandbox-rpm-prod"
...
function assert_DEBIAN_REPO_BASE_URL {
  ...
  local msg="DEBIAN_REPO_BASE_URL for (USE_TEST_REPO=$USE_TEST_REPO, ENVIRONMENT=$ENVIRONMENT) should be $expected"
  assert_eq "$expected" "$DEBIAN_REPO_BASE_URL" "$msg" && log_success "$msg" || TESTS_RESULT=$?
}

log_header "Tests for DEBIAN_REPO_BASE_URL"
assert_DEBIAN_REPO_BASE_URL "true" "live" "https://dummy_user:dummy_token@repository.hazelcast.com/debian-test-local"
assert_DEBIAN_REPO_BASE_URL "true" "sandbox" "https://dummy_user:dummy_token@repository.hazelcast.com/debian-test-local"
assert_DEBIAN_REPO_BASE_URL "false" "live" "https://repository.hazelcast.com/debian-local"
assert_DEBIAN_REPO_BASE_URL "false" "sandbox" "https://dummy_user:dummy_token@repository.hazelcast.com/sandbox-deb-prod"
...
function assert_BREW_GIT_REPO_NAME {
  ...
  local msg="BREW_GIT_REPO_NAME for (USE_TEST_REPO=$USE_TEST_REPO, ENVIRONMENT=$ENVIRONMENT) should be $expected"
  assert_eq "$expected" "$BREW_GIT_REPO_NAME" "$msg" && log_success "$msg" || TESTS_RESULT=$?
}

log_header "Tests for BREW_GIT_REPO_NAME"
assert_BREW_GIT_REPO_NAME "true" "live" "hazelcast/homebrew-hz-test"
assert_BREW_GIT_REPO_NAME "true" "sandbox" "hazelcast/homebrew-hz-test"
assert_BREW_GIT_REPO_NAME "false" "live" "hazelcast/homebrew-hz"
assert_BREW_GIT_REPO_NAME "false" "sandbox" "hazelcast/homebrew-sandbox-hz"
