...
# Checks if we should build the OSS artifacts.
# Returns "yes" if we should build it or "no" if we shouldn't.
#
# - If the workflows was triggered by `pull_request` OSS artifact should be built always
#
# If the workflow was triggered by "push" (usually tag push) we should build an OSS artifacts only with these conditions fulfilled:
...
#
# Check test cases in `build.functions_tests.sh` to see the examples
...
# Checks if we should build the EE artifacts.
# Returns "yes" if we should build it or "no" if we shouldn't.
#
# - If the workflows was triggered by `pull_request` EE artifact should be built always
...
function should_build_ee() {
  ...
  if [[ $editions == "All" || $editions == "EE" ]]; then
    echo "yes"
    return
  fi
