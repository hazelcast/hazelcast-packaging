...
export PACKAGE_VERSION=$(echo $PACKAGE_VERSION | sed -r -r 's/(-)/\./g')
...
mv ${HZ_DISTRIBUTION}-distribution-${HZ_VERSION}.tar.gz build/rpmbuild/SOURCES/${HZ_DISTRIBUTION}-${HZ_VERSION}.tar.gz
...
rpm --define "_gpg_name deploy@hazelcast.com" --addsign build/rpmbuild/RPMS/noarch/${HZ_DISTRIBUTION}-${PACKAGE_VERSION}-1.noarch.rpm

if [ "${PUBLISH}" == "true" ]; then

  derive_package_repo "${HZ_VERSION}"

  RPM_SHA256SUM=$(sha256sum build/rpmbuild/RPMS/noarch/${HZ_DISTRIBUTION}-${PACKAGE_VERSION}-1.noarch.rpm | cut -d ' ' -f 1)
  RPM_SHA1SUM=$(sha1sum build/rpmbuild/RPMS/noarch/${HZ_DISTRIBUTION}-${PACKAGE_VERSION}-1.noarch.rpm | cut -d ' ' -f 1)
  RPM_MD5SUM=$(md5sum build/rpmbuild/RPMS/noarch/${HZ_DISTRIBUTION}-${PACKAGE_VERSION}-1.noarch.rpm | cut -d ' ' -f 1)
